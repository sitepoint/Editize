...
/**
 * Reimplements (with slight modifications) the javax.swing.text.DefaultCaret
 * class in Java 1.3.1. The moveDot(int,Position.Bias) method in that class is
 * package private, which prevents overriding required by Editize. Hopefully in
 * a future version, this will be fixed. See Sun Java Bug#4250070 for details.
 *
 * @author  Kevin Yank
		...
		MouseListener, MouseMotionListener
{
	...
	if ((e.getModifiers() & InputEvent.BUTTON1_MASK) != 0) {
		if(e.getClickCount() == 2) {
			Action a = new EditizeEditorKit.SelectWordAction();
			a.actionPerformed(null);
		} else if(e.getClickCount() == 3) {
			Action a = new EditizeEditorKit.SelectLineAction();
			a.actionPerformed(null);
		}
	}
	}
	...
	if (dot != this.dot) {
		...
		if (h != null) {
			...
			//throw new StateInvariantError("Bad caret position");
				throw new Error("Bad caret position");
			}
		}
		}
	}
